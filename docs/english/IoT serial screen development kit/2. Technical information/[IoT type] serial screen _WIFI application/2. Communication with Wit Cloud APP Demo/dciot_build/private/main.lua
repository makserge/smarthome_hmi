...
function gagent_get_info()  
 ...
 --product_key = 9261d41530c44d55a65a25bb177b1939 
...
function update_cloud_ui()
	...
	set_value(3,1,switch)                                                 --控制控件
	set_value(3,2,switch_plasma)
	set_value(3,3,led_air_quality)
	set_value(3,4,child_security_lock)
	set_value(3,10,wind_velocity)
	
	filter_life = dev_status[2]
	set_value(3,11,filter_life)
	set_value(3,12,filter_life)
	
	week_repeat = dev_status[3]
	set_value(3,13,week_repeat)
	set_value(3,14,week_repeat)
	
	temprature = dev_status[4]*255+dev_status[5]
	temprature = temprature/10.0
	set_value(3,6,temprature)
	
	countdown_off_min = dev_status[6]*256+dev_status[7]
	set_value(3,16,countdown_off_min)	
end
	 ...
	 if screen==1 and control==7 or 
	  ...
	  then
	    scan_ap_fill_list()
	  	 ...
	  	 if screen==1 and control==8                                           --保存设置
	 then
	   ssid = get_text(1,4)
	   psw = get_text(1,5)
